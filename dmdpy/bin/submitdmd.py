...
from ..utility import utilities, constants
...
def main(_cores: int=None, _time: int=None, _nodes: int=1, _sub: bool=True):
    ...
    if cores is None or time is None:
        ...
        parser = argparse.ArgumentParser(description="Submits a DMD simulation to a queuing system")
    ...
    path_to_template = os.path.join(home, ".config/dmdpy")
    
    if not os.path.isdir(path_to_template):
        path_to_template = os.path.join(home, ".dmdpy")
